...
class ReferenceCCA_IPLS(ElasticCCA):
    r"""
...
#class _ElasticInnerLoop():
    ...
    def _update_view(self, views, view_index: int): #2 - Function to calculate gradients, and update weight and scores
        """
        ...
        self.scores[view_index] = views[view_index] @ self.weights[view_index] #Update scores
    ...
    def gradient_update_X_ref(self,X, T, U, V, l1_ratio, lr): #Calculate gradients for X
    ...
    def gradient_update_Y_ref(self,T, Y, U, V, l1_ratio, lr): #Calculate gradients for Y
    ...
    def _objective(self, views): #3- Calulate loss from loss function. Loops for each modality
        c = np.array(self.c)
        ...
        for i, _ in enumerate(views):
            ...
            if self.reference is not None:
                l1_pen = l1[i] * (np.linalg.norm((views[i] @ self.weights[i] - self.reference))** 2 / (
                    2 * self.n)
                )
            else:
            ...
            total_objective += objective + l1_pen

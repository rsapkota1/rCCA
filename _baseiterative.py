...
from rccaFunctions import _ReferenceCCA
from cca_zoo.models._rcca import rCCA


class _BaseIterative(_ReferenceCCA):
    """
    ...
    def fit(self, views: Iterable[np.ndarray], y=None, reference=None, **kwargs): #When doing CV - 1
        """
        ...
        if len(views)>2:
            if reference is None:
                self.reference = views[-1] #Add third value to reference
                self.reference = self.reference.squeeze(1)

                views=views[:-1] #Remove reference from views
            else:
